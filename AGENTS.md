...
This repository is documentation-first, with a static public site generated from Astro source. Do not commit generated build output.
...
- `site-src/` contains the GitHub Pages source site. Astro renders localized static HTML; React islands handle catalog filters and copy controls.
- `dist/` is generated by `npm run build` and must stay uncommitted.
- `nonviolent-skills-research.md` and `civil-resistance-evidence-and-critique.md` are research notes.

## Build, Test, and Development Commands

Use the site build and lightweight repository validation commands before committing:
...
Use `rg` to search content and verify stale names or legacy structure are not reintroduced. For registry edits, manually verify method counts and table structure. For site edits, verify the generated `dist/` output locally but do not commit it.

## Coding Style & Naming Conventions

Write Markdown in concise English. Use ATX headings (`#`, `##`) and avoid decorative separators unless they add meaning.

Site source uses Astro plus React islands. Keep SEO-critical copy in Astro-rendered data/templates so localized page content exists in static HTML before hydration. Use React only for interactive catalog behavior.
