...
async function checkLivePages() {
  for (const path of pages) {
    ...
    for (const lang of ["en", "ru", "es", "pl", "x-default"]) {
...
async function checkCrawlerFiles() {
  ...
  if (urlCount !== 8) fail(`sitemap URL count is ${urlCount}, expected 8`);
  else ok("sitemap has 8 URL entries");
  if (hreflangCount !== 40) fail(`sitemap hreflang count is ${hreflangCount}, expected 40`);
