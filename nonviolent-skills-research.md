...
5. **Naming scheme (per `methods-registry/README.md` and `methods-registry/00-naming-and-taxonomy.md`):** atomic methods use code-first four-digit IDs without a duplicated domain slug. Top-level groups use `1000`, `2000`, ... `6000`; subgroups use `1100`, `1200`, etc.; concrete method skills use IDs such as `4108-public-speeches`. Domain and subgroup live in metadata fields (`domain_group`, `domain_subgroup`). Registry/support documents use separate two-digit file prefixes, currently `00` through `03`.
