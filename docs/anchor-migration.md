...
2. **Visible "moved" notice.** When the content moved off the homepage
   entirely, the homepage keeps a small stub section at the same `id` that
   names the new destination and links to it. The stub is visible on the
   homepage at all times, so the destination is always one click away. This
   approach is keyboard-accessible, works without JavaScript, and meets
   WCAG 2.2.1 (Timing Adjustable) since there is no timed behaviour to
   cancel or extend.
...
| `#domains` | Inline domain grid | Stubbed: visible "moved" notice | `/domains/` (and the locale equivalents) |
| `#catalog` | Inline method catalogue (CatalogTable) | Stubbed: visible "moved" notice | `/methods/` (and the locale equivalents) |
